...
#### B-1. 글 저장 (초안 CRUD)
- [ ] Post 테이블 저장/수정/삭제 API
- [ ] 글 목록 조회 페이지 (상태별 필터, 검색)
- [ ] 글 상세/편집 페이지
- [ ] 글 생성 시 자동 초안 저장

#### B-2. 발행 이력 조회
- [ ] PublishLog 기반 발행 목록 (성공/실패, 플랫폼, URL)
- [ ] 발행 상세 (에러 메시지 확인)

#### B-3. 설정 저장 기능
- [ ] 글쓰기 설정 저장 (Setting 모델 활용)
- [ ] AI 설정 저장 (Claude/DALL-E/Unsplash 설정)
- [ ] 설정 불러오기 (페이지 로드 시 DB에서 조회)
