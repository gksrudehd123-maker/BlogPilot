...
- [x] 블로그스팟 연결테스트 기능
- [x] 블로그스팟 글 발행 (Phase 3에서 구현 완료)

#### 2-3. 워드프레스 연동 (완료)
- [x] 워드프레스 REST API 클라이언트 (Application Password 인증)
- [x] 워드프레스 연결테스트 기능
- [x] 워드프레스 글 발행 (Phase 3에서 구현 완료)

#### 2-4. 네이버/티스토리 계정 저장 (완료)
- [x] 네이버/티스토리 계정 정보 저장 UI 동작 (발행은 Phase 9)
- [x] 네이버/티스토리 연결테스트 안내 메시지 (데스크톱 앱 필요)

#### 2-5. 공통 어댑터 패턴 리팩토링
- [ ] 플랫폼별 어댑터 패턴 추출 → Phase 3에서 발행 구현 후 리팩토링
...
#### 3-2. Claude API 연동 (완료)
- [x] Claude API 클라이언트 구현 (키워드 + 프롬프트 → 블로그 글 생성, fetch 방식)
- [x] 글 생성 API (/api/posts/generate)
- [x] Claude API 연결 테스트 완료 (API 키 재발급으로 해결)

#### 3-3. 글쓰기 페이지 UI (완료)
- [x] 키워드 입력 + 프롬프트 선택 + 톤/글자수 설정
- [x] AI 글 생성 + 미리보기/편집 + 복사 기능
- [x] 발행 플랫폼 선택 UI (연동된 블로그스팟/워드프레스 목록)
- [ ] 글 저장 (초안 — Post 테이블) → Phase 4-2에서 구현

#### 3-4. 발행 기능 (완료)
- [x] 블로그스팟 발행 (Blogger API) — 발행 테스트 완료
- [x] 워드프레스 발행 (REST API) — 발행 테스트 완료
- [x] 발행 API (/api/publish) + 플랫폼 선택 UI
- [x] 발행 결과 표시 (성공 URL / 실패 메시지)
- [x] 발행 로그 DB 저장 (PublishLog 테이블)

### Phase 4 - 네이버/티스토리 발행 + 코드 정리
> 네이버 블로그와 티스토리는 API 미지원/종료로 Playwright 브라우저 자동화로 구현.
> 4개 플랫폼 발행 모두 확인 후 공통 어댑터 패턴으로 리팩토링.

#### 4-1. 네이버 블로그 Playwright 자동 발행
- [ ] Playwright 설치 및 설정
- [ ] 네이버 로그인 (쿠키 저장 방식 — 최초 수동 로그인 후 세션 유지)
- [ ] 네이버 블로그 글쓰기 자동화 (제목/본문 입력 → 발행)
- [ ] 발행 테스트 완료

#### 4-2. 티스토리 Playwright 자동 발행
- [ ] 티스토리(카카오) 로그인 자동화 (쿠키 저장 방식)
- [ ] 티스토리 글쓰기 자동화 (제목/본문 입력 → 발행)
- [ ] 발행 테스트 완료

#### 4-3. 코드 정리
- [ ] 글 저장 (초안 — Post 테이블)
- [ ] 4개 플랫폼 공통 어댑터 패턴 리팩토링

### Phase 5 - 콘텐츠 품질 향상 (프롬프트 + 이미지)
> 프롬프트 고도화로 글 품질을 높이고, AI 이미지를 글 중간에 자동 삽입.
...
### Phase 10 - 데스크톱 앱 (Tauri)
> Tauri로 .exe 데스크톱 앱 빌드. 네이버/티스토리 자동화는 Phase 4에서 Playwright로 선행 구현.
