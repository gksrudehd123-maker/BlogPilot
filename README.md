...
#### B-1. 글 저장 (초안 CRUD)
- [ ] 글 CRUD API (POST/GET/PUT/DELETE /api/posts)
- [ ] 글 목록 페이지 /posts (테이블, 상태 배지, 검색/필터)
- [ ] 글 상세/편집 페이지 /posts/[id] (수정 후 재발행 가능)
- [ ] 기존 /posts → /posts/new로 이동 (새 글 쓰기)
- [ ] AI 생성 시 자동 DRAFT 저장 (유실 방지)

#### B-2. 발행 이력 조회
- [ ] 글 상세 페이지에 PublishLog 표시 (플랫폼, 시간, 성공/실패, URL, 에러)
- [ ] 글 목록에 발행 상태 배지
...
#### B-4. 설정 저장
- [ ] Setting 모델에 설정값 저장/조회 API
- [ ] 글쓰기 설정, AI 설정 페이지 저장 버튼 동작
- [ ] 페이지 로드 시 DB에서 설정 불러오기
